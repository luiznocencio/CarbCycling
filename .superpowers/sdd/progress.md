...
- Task 5 (endpoints peso + apply-adjustment + profile): complete (commit 3306b7e). GET/POST /api/weight (POST retorna {log, analysis}); DELETE /api/weight/[id] (resync); POST /api/weight/apply-adjustment (soma delta em kcal_adjustment + recalcTargetsForUser). Profile PUT preserva kcal_adjustment (lê atual) e registra peso via recordWeight (weight_kg = espelho do último log). tsc+56 verdes.
- Task 6 (UI /weight): complete (commit 32c2ff0). WeightTracker (form+gráfico SVG+histórico+card de tendência/sugestão 1 clique) + página Progresso + link no nav. Testids: weight-form/weight-input/weight-date/weight-note/weight-save/weight-trend/weight-apply-adjustment/weight-chart/weight-history/weight-delete. tsc+56 verdes.
- Task 7 (E2E peso + apply-adjustment): complete (commit pending, controller). Bloco 10: POST /api/weight (data antiga 82 + hoje 79.6) → histórico>=2 e profiles.weight_kg=79.6 (espelha o mais recente=hoje); apply-adjustment delta -150 → kcal_adjustment=-150; página /weight carrega (weight-form+weight-history). unit 56 + e2e 1 verdes.
FIX (nav overflow mobile): o 5º link "Progresso" estourava o nav em 375px (flex sem shrink/wrap) → overflow horizontal quebrava cliques em várias páginas do E2E (interceptação de pointer events). Corrigido em layout.tsx: logo/SignOut shrink-0, container de links min-w-0 overflow-x-auto whitespace-nowrap. E2E verde sem force. (Regressão real de mobile, não só teste.)
D CODE COMPLETE (Tasks 1-7) on branch feat/d-peso-evolucao. Sem IA. Migração aditiva. Refactor recalc preservou contrato (E2E bloco 2 verde).
Final review (controller self-review): sem Crítico/Importante. (1) Refactor recalc preservou contrato (E2E bloco 2 verde) e passou a usar kcal_adjustment (default 0 = igual). (2) Lógica de tendência pura/testada (mínimos quadrados; expected atrelado ao plano via 7700 kcal/kg; limiar 75; arredonda 25; direções eat_less/eat_more/on_track/insufficient). (3) weight POST calcula analysis com inputs corretos (bmr→tdee, goalAdjustment, kcal_adjustment, logs, asOf); GET/DELETE RLS + resync. (4) apply-adjustment valida delta (int, |Δ|≤1000), soma, recalc; retorna {kcal_adjustment,summary}. (5) profile PUT preserva kcal_adjustment e registra peso (weight_kg=espelho do último log). (6) Travas herdadas (distributeWeeklyTargets). Smoke prod = manual pós-deploy (registrar peso + tendência + apply). PRONTO PARA MERGE. Menor: apply-adjustment não reverte kcal_adjustment se recalc falhar (benigno single-user; aplica no próximo recalc). Nav overflow mobile corrigido no caminho.
MERGED to main (ff, 3607239..1a71d06) + pushed; Vercel deploy dpl_23v11 READY (commit 1a71d06, alias carb-cycling-seven.vercel.app). Sem segredo no diff.
PROD SMOKE D (endpoints reais): usuário @example.com, perfil fat_loss/moderate (peso 82), tipo low + padrão semanal. Pesos 82→81.5 ao longo de ~3 sem (perda lenta). POST /api/weight retorna analysis: expected -0.50 kg/sem, actual -0.12 kg/sem → suggestedDelta -425, status eat_less (perdendo devagar → comer menos). apply-adjustment -425: kcal_adjustment=-425, avgDailyTarget 2211→1786, metas recalculadas. profiles.weight_kg=81.5 (espelho do último log=hoje). Página /weight renderiza (form+gráfico+histórico 4 registros). Usuários @example.com + weight_logs limpos (0; gmail real intacto). FEATURE D LIVE.

=== UX pass — nav responsivo + semana no desktop (branch fix/ux-nav-week-responsive, merged) ===
Feedback do usuário: desktop "Semana" com formatação quebrada (difícil ver dados); mobile com rolagem lateral no menu.
Causa: shell inteiro em max-w-3xl (768px) → WeekGrid xl:grid-cols-7 espremido; nav com overflow-x-auto (paliativo) → scroll lateral.
Fix: (1) AppNav.tsx novo (client, usePathname p/ rota ativa) — barra superior no desktop (md+), tab bar inferior fixa com ícones SVG no mobile (Semana/Progresso/Alimentos/Prefs./Ajustes); topo mobile só logo+Sair. (2) Largura movida do layout p/ cada página: layout wrapper max-w-6xl + pb-24 md:pb-8 (espaço p/ tab bar); dashboard usa largura cheia (7 dias numa linha), forms (settings/preferences/weight/foods/day) com mx-auto max-w-3xl. (3) WeekGrid card: números arredondados (Math.round), linha kcal limpa "612 / 1700 kcal", "Fora da meta" discreto embaixo (barra vermelha já sinaliza). tsc 0 + unit 56 + e2e 1 passed (tab bar não intercepta cliques; force do "+ Novo tipo" já removido). Verificado por screenshots desktop+mobile. Merged ff f42e1fc..25c234c, deploy dpl_FevU READY, alias carb-cycling-seven.vercel.app.

=== Feature: coerência culinária do cardápio — branch feat/coerencia-cardapio ===
Spec: docs/superpowers/specs/2026-08-03-coerencia-cardapio-design.md
Plan: docs/superpowers/plans/2026-08-03-coerencia-cardapio.md
BASE (branch a partir da main): 5b40fa4. Spec b6b48b5, plan 33739cc.
NOTE: sem migração, sem IA nova. Guarda subtrativa (enforceCoherence) roda antes do solver. Módulo puro coherence.ts compartilhado por generate (C1/E2) e suggest-option (C2). Decisões: 1 proteína animal principal (ovo/feijão/laticínio = complemento); arroz+feijão permitido; carbo-base (carbo+pao) limitado a 1.
- Tasks 1-4 (coherence.ts puro): complete (commits 604e40b, e7e8ae4, 018443f, 32fdc72). classifyFood/mealTypeFromName/coherenceGuidance+coherenceRulesGeneral/enforceCoherence. Implementer achou+corrigiu bug do plano (atum caía em gordura porque "oleo" casava antes de "atum" → moveu gordura p/ depois de proteina_animal). norm() com ̀-ͯ. Revisado pelo controller (módulo limpo). unit 67 verdes, tsc 0.
- Task 5 (prompt menu.ts): complete (commit 7992c52). generateMenu/suggestMealOption com coherenceRulesGeneral+tipo/molde+categoria dos alimentos; PoolFood ganha categoria via classifyFood; mealType opcional. tsc 0.
- Task 7 (nomes n=3,4,6 solver): complete (commit e39c6d4). NAMES_BY_N/WEIGHTS_BY_N (n=5 pesos idênticos); ajustou teste pré-existente de n=3 (comportamento mudou de propósito) + teste de fallback n=7. unit 69 verdes.
- Task 6 (wiring generate.ts + suggest-option): complete (commit 38e3252). enforceCoherence entre withFood e scaleOptionToTarget (protein_g/carbs_g via itemGrams); mealType via mealTypeFromName passado aos geradores. tsc 0, unit 69, E2E 1 passed. Semana (E2) herda por reuso.
Final review (controller): wiring correto nos dois pontos (guarda antes do solver), prompt com regras+tipo/molde+categoria. Sem Crítico/Importante. CODE COMPLETE. Falta smoke IA real (E2E é determinístico) + merge/deploy.
SMOKE IA REAL (local, branch): usuário @example.com, tipo médio, generate 4 refeições × 2 opções. Resultado ZERO violações: café=Pão+Iogurte+Banana / Pão+Ovo+Maçã; almoço/jantar=Frango+Arroz+Feijão+Brócolis+Azeite / Carne+Arroz+Feijão+Tomate; lanche=fruta+iogurte+amendoim. Nunca frango+carne juntos; arroz+feijão preservado; adequado ao horário. Feature valida.
PENDENTE: Supabase MCP desconectou nesta sessão → não deu p/ limpar o usuário smoke_coh_20260803@example.com (pegada mínima: 1 profile + 1 day type; generate não persiste). Limpar quando o conector voltar.
CODE COMPLETE — aguardando OK do usuário p/ merge+deploy.
